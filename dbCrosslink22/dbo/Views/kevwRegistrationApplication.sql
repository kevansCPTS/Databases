...
CREATE VIEW [dbo].[kevwRegistrationApplication]
AS
	select        
		vba.AccountID
	,   vba.UserID
	...
	from
		(
			...
			,   vba1.EfinPrint
			,   vba1.EfinProduct2
			,	row_number() over ( partition by vba1.efin, vba1.bankId order by vba1.BankAppId desc) AS 'RowNumber' 
			from
				dbo.vwBankApplication vba1 join dbo.ltblRegistrationStatusDescription rsd on vba1.Registered = rsd.Registered
					and rsd.SendToEFINMaster = 1
		) vba
	where
		vba.RowNumber = 1
