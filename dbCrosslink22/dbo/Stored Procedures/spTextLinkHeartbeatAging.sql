...
-- Description:	Make sure that TextLink windows service is functioning and responsive by monitoring a heartbeat indicator. TextLink is supported for two years, current year and prior year.
-- =============================================
CREATE PROCEDURE [dbo].[spTextLinkHeartbeatAging]
...
BEGIN
	...
	SET @EmailBody = 'The following Textlink windows service app thread''s heartbeats are flatlining: ';
	SELECT @FailThreshold = value FROM tblServerAppConfiguration WHERE pk_key = 'textlink.service.heartbeat.failure.threshold' --IN MINUTES, maximum heartbeat lag before an error email is sent
	...
	SELECT @HeartbeatQueue = value FROM tblServerAppConfiguration WHERE pk_key = 'textlink.service.heartbeat.queueworker'
