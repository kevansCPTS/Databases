CREATE procedure [ke_pprGenFeeMgmtRec_ver2] --'BRASUN',60392,0
    @account                varchar(8)
,   @userId                 int
,   @efin                   int
as
        ...
        begin
            select 
                @pXml = case @prod_cd 
                    ...
                    + '<PPIA>' + convert(varchar(10),isnull(mu.fedMarkUp,0) * 100) + '</PPIA>'
                    + '<PPIB>' + convert(varchar(10),isnull(mu.stateMarkUp,0) * 100) + '</PPIB>'

                    + '<PPIC>' + convert(varchar(10),isnull(fa.feeAmount,0) * 100) + '</PPIC>'
                    + '<PPID>' + convert(varchar(10),isnull(sa.feeAmount,0) * 100) + '</PPID>'

                    + '<PPBF>' + convert(varchar(10),isnull(bf.busFedBaseFee,0) * 100) + '</PPBF>'
                    + '<PPBS>' + convert(varchar(10),isnull(bf.busStateBaseFee,0) * 100) + '</PPBS>'

                    + '<PPBA>' + convert(varchar(10),isnull(mu.busFedMarkUp,0) * 100) + '</PPBA>'
                    + '<PPBB>' + convert(varchar(10),isnull(mu.busStateMarkUp,0) * 100) + '</PPBB>'

                    + '<PPBC>' + convert(varchar(10),isnull(bfa.feeAmount,0) * 100) + '</PPBC>'
                    + '<PPBD>' + convert(varchar(10),isnull(bsa.feeAmount,0) * 100) + '</PPBD>'
