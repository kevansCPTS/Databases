...
-- Description:	Make sure that the RemoteSignWorker windows service is functioning and responsive by monitoring a heartbeat indicator.
-- =============================================
CREATE PROCEDURE [dbo].[up_RemoteSignWorkerHeartbeatAging]
...
BEGIN
	...
	SET @EmailBody = 'The following RemoteSignWorker windows service app thread''s heartbeats are flatlining: ';
	SELECT @FailThreshold = value FROM tblServerAppConfiguration WHERE pk_key = 'remotesignworker.service.heartbeat.failure.threshold' --IN MINUTES, maximum heartbeat lag before an error email is sent
	...
	SELECT @HeartbeatQueue = value FROM tblServerAppConfiguration WHERE pk_key = 'remotesignworker.service.heartbeat'
