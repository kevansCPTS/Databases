...
CREATE PROCEDURE [dbo].[up_rebuildDailyFilingActivity]
AS
...
declare @cdate date = getdate()
	...
	where  
        tm.isFullyFunded = 1
        and tm.fullyFundedDate is not null
	group by
        u.account
    ,   tm.[User_ID]
    ,   tm.EFIN
        ...
        #TempDailyActivity tdfa join (
                                        ...
                                        from 
                                            dbo.tblCorpmast cm join dbCrosslinkGlobal.dbo.tblUser u on cm.userid = u.[user_id]
                                                ...
                                                and cm.subType in ('EA' COLLATE Latin1_General_CS_AS ,'EC' COLLATE Latin1_General_CS_AS ) 
                                        group by
                                            u.account
                                        ,   cm.userid
                                        ,   cm.efin
    ...
    ,   StateBusinessAcks
        ...
        from 
            dbo.tblCorpmast cm join dbCrosslinkGlobal.dbo.tblUser u on cm.userid = u.[user_id]
                and cm.ldate is not null
                and cm.subType in ('EA' COLLATE Latin1_General_CS_AS ,'EC' COLLATE Latin1_General_CS_AS ) 
            left join #TempDailyActivity tdfa on u.account = tdfa.account
                and cm.UserID = tdfa.userid
    ...
    ,   StateBusinessAcks
        ...
        select
            ...
            ,   tdfa.FedBusinessReturns
            ,   tdfa.FedBusinessAcks
            ,   tdfa.StateBusinessReturns
            ,   tdfa.StateBusinessAcks
        from
