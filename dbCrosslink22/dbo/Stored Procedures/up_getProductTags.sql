/****** Object:  StoredProcedure [dbo].[up_getProductTags]    Script Date: 5/8/2019 12:42:26 PM ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[up_getProductTags] 
		...
		IF (@@FETCH_STATUS <> -2)
			BEGIN 
				...
				if (@isEROAddOn = 1 and ISNULL(@EROAddOnAmount, 0.00) > 0.00 and @agreeToParticipate = 1 and @userID >= 500000 and @userID < 900000)
					insert into admin (req_type,ssn,param,requestor) values ('T', @userID, @Tag + 'A' + convert(varchar(40), replace(floor(IsNull(@EROAddOnAmount,0)*100), '.00','')), 'gpt')
				else if (@userID >= 500000 and @userID < 900000)
					insert into admin (req_type,ssn,param,requestor) values ('T', @userID, @Tag + 'A', 'gpt')
				...
				if (@isBaseAddon = 1 and @agreeToParticipate = 1 and @userID >= 500000 and @userID < 900000)
					insert into admin (req_type,ssn,param,requestor) values ('T', @userID, @Tag + 'B' + convert(varchar(40), replace(floor((IsNull(@BasePrice,0) + IsNull(@BasePriceAddon,0))*100), '.00','')), 'gpt')
				else if (@agreeToParticipate = 1 and @userID >= 500000 and @userID < 900000)
					insert into admin (req_type,ssn,param,requestor) values ('T', @userID, @Tag + 'B' + convert(varchar(40), replace(floor(IsNull(@BasePrice,0)*100), '.00','')), 'gpt')
				else if (@userID >= 500000 and @userID < 900000)
					insert into admin (req_type,ssn,param,requestor) values ('T', @userID, @Tag + 'B', 'gpt')
				...
				IF (@isAutoAdd = 1 and (@typeAvailability = 3 or @typeAvailability = 1) and @autoAddFinancial = 1 and @agreeToParticipate = 1 and @userID >= 500000 and @userID < 900000)
					insert into admin (req_type,ssn,param,requestor) values ('T', @userID, @Tag + 'FX', 'gpt')
				else if (@userID >= 500000 and @userID < 900000)
					insert into admin (req_type,ssn,param,requestor) values ('T', @userID, @Tag + 'F' , 'gpt')

				IF (@isAutoAdd = 1 and @typeAvailability = 3 and @autoAddNonFinancial = 1)
				...
				IF (@isAutoAdd = 1 and @typeAvailability = 3 and @autoAddNonFinancial = 1 and @agreeToParticipate = 1 and @userID >= 500000 and @userID < 900000)
					insert into admin (req_type,ssn,param,requestor) values ('T', @userID, @Tag + 'NX', 'gpt')
				else if (@userID >= 500000 and @userID < 900000)
					insert into admin (req_type,ssn,param,requestor) values ('T', @userID, @Tag + 'N', 'gpt')
				...
				if (@userID >= 500000 and @userID < 900000)
					insert into admin (req_type,ssn,param,requestor) values ('T', @userID, @Tag + 'L' + @TempString, 'gpt')

				if (@userID >= 500000 and @userID < 900000)
					insert into admin (req_type,ssn,param,requestor) values ('T', @userID, @Tag + 'I' + dbo.udf_getAncillaryProdBankApprovalTag(@ProdID), 'gpt')
